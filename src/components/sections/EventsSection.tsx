      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3
              className={`font-display font-bold text-sm ${theme === 'dark' ? 'text-white' : 'text-light-text'
                }`}
            >
              {title}
            </h3>
            <span
              className={`text-xs flex-shrink-0 font-display ${theme === 'dark' ? 'text-slate-500' : 'text-light-muted'
                }`}
            >
              {date}
            </span>
          ...
          <p
            className={`text-xs leading-relaxed mb-2 ${theme === 'dark' ? 'text-slate-400' : 'text-light-muted'
              }`}
          >
            {desc}
          </p>
...
const EventsSection: React.FC = () => {
  ...
  // ✅ safe parsing helper
  const upcomingEvents = (t('events.upcoming_list', { returnObjects: true }) as EventItem[]) || []
  const pastEvents = (t('events.past_list', { returnObjects: true }) as EventItem[]) || []

  const labels = t('events.labels', { returnObjects: true }) as {
    upcoming: string
    past: string
  }
  ...
  return (
    <section id="events" className="pt-10 relative z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        >
          <span
            className={`font-display text-xs tracking-widest uppercase ${theme === 'dark' ? 'text-cyber-purple' : 'text-purple-500'
              }`}
          >
            {t('events.subtitle')}
          </span>
