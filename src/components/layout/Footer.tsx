...
const Footer: React.FC = () => {
  ...
  return (
    <footer className={`relative z-10 border-t ${
      theme === 'dark' ? 'border-cyber-border/40 bg-cyber-black/90' : 'border-light-border bg-white/90'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div className="flex items-center gap-3 mt-6">
              {socialLinks.map(({ icon: Icon, href, label, color }) => (
                <a
                  ...
                  aria-label={label}
                  className={`p-2.5 rounded-lg border transition-all duration-200 group ${
                    theme === 'dark'
          ...
          {/* Quick Links */}
          <div>
            <h3 className={`font-display text-xs tracking-widest uppercase mb-4 ${
        ...
        <div className={`mt-10 pt-6 border-t flex flex-col sm:flex-row items-center justify-between gap-4 ${
          ...
          <div className="flex items-center gap-4">
            <a href="#" className={`text-xs transition-colors duration-200 ${
              theme === 'dark' ? 'text-slate-500 hover:text-cyber-teal' : 'text-light-muted hover:text-light-blue'
            }`}>
              {t('footer.privacy')}
            </a>
            <a href="#" className={`text-xs transition-colors duration-200 ${
