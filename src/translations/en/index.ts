const en = {
  nav: {
    home: 'Home',
    articles: 'Articles',
    impact: 'Our Impact',
    collaborate: 'Collaborations',
  },
  hero: {
    badge: '[ ONLINE ] // Israel Cyber Community',
    title: 'CyberIsrael',
    subtitle: 'Connecting people who live and breathe cyber and technology.',
    tagline: 'Community · Learning · CTFs · Conferences · Collaborations',
    cta_join: 'Join the Community',
    cta_explore: 'Explore More',
    scroll: 'Scroll to explore',
  },
  about: {
    title: 'About Us',
    subtitle: 'We are a community',
    description: 'Building Israel\'s cyber community',
    stat_members: 'Community Members',
    stat_ctfs: 'CTF Participants',
    stat_events: 'Events & Projects',
    stat_years: 'Years Growing',

  },
  values: {
    title: 'Our Values',
    subtitle: 'What we stand for',
    val1_title: 'Connect',
    val1_desc: 'Creating meaningful professional and social connections between people from all backgrounds and experience levels in cyber and technology.',
    val2_title: 'Improve',
    val2_desc: 'Learning together through technical content, hands-on projects, competitions, workshops, and knowledge sharing.',
    val3_title: 'Expose',
    val3_desc: 'Making the cyber world accessible to everyone by opening the door for curious newcomers and future talent.',
  },
  culture: {
    title: 'The Culture We\'re Building',
    subtitle: 'A shared approach to learning, professionalism, collaboration, and growth in cyber.',
    app1_title: 'Accessible Knowledge',
    app1_desc: 'We believe knowledge should be available to everyone — from complete beginners to experienced professionals.',
    app2_title: 'Growth Through Practice',
    app2_desc: 'Real learning comes from building, solving, competing, and experimenting together.',
    app3_title: 'People First',
    app3_desc: 'CyberIsrael is built around community, support, collaboration, and helping others grow.',
    app4_title: 'Professional Excellence',
    app4_desc: 'We strive to raise the level of technical knowledge and professionalism across the Israeli cyber community.',
  },
  events: {
    title: "Events & Activities",
    subtitle: "What we build together",

    labels: {
      upcoming: "Upcoming Events",
      past: "Past Events",
      conference: "Conference",
      ctf: "CTF Competition",
      workshop: "Workshop",
    },

    upcoming_list: [
      {
        type: "workshop",
        title: "Online Community Learning Sessions / Lectures",
        desc: "Online exposure sessions to the Cyber and Development world",
        date: "Very soon!!!",
      },
    ],

    past_list: [
      {
        type: "conference",
        title: "CyberIsrael Conference",
        desc: "Professional cyber conference hosted at Bar-Ilan University featuring expert lectures across multiple topics and skill levels.",
        date: "Mar 2025",
      },
      {
        type: "ctf",
        title: "CyberIsrael x BIU Cyber Club CTF",
        desc: "Large-scale online Capture The Flag competition held in collaboration with the Bar-Ilan University Cyber Club.",
        date: "Jan 2025",
      },
    ],
  },
  social: {
    title: 'Connect With Us',
    subtitle: 'Find us everywhere',
    discord: 'Discord Server',
    whatsapp: 'WhatsApp Group',
    instagram: 'Instagram',
    tiktok: 'TikTok',
    email: 'Email',
    join_discord: 'Join our Discord',
    join_whatsapp: 'Join WhatsApp',
    drive: 'Community Drive With Resources for you!',
  },
  join: {
    title: 'Join CyberIsrael',
    subtitle: 'A community for everyone in cyber and technology',
    description: 'Whether you are a startup founder, industry professional, student, soldier, researcher, or someone just starting their journey — CyberIsrael is for you. Join over 1,700 members building the future of cyber in Israel together.',
    cta: 'Join Now',
    discord_cta: 'Join Discord',
    feature1: 'Access community events and activities',
    feature2: 'Meet people from across the industry',
    feature3: 'Learn through shared knowledge and projects',
    feature4: 'Be part of Israel\'s growing cyber community',
  },
  articles: {
    title: 'Articles',
    subtitle: 'Knowledge from the community',
    read_more: 'Read More',
    min_read: 'min read',
    by: 'By',
    categories: {
      all: 'All',
      web: 'Web Security',
      pwn: 'Pwn & Exploit',
      crypto: 'Cryptography',
      forensics: 'Forensics',
      malware: 'Malware Analysis',
      osint: 'OSINT',
      ctf: 'CTF Writeup',
    },
  },
  impact: {
    title: 'Our Impact',
    subtitle: 'Shaping the cyber landscape',
    ctf_section: 'CTF Competitions',
    ctf_desc: 'Organizing and participating in competitions that encourage learning, teamwork, and technical growth.',
    conferences_section: 'Community Conferences',
    conferences_desc: 'Hosting physical and online events that connect people through knowledge and collaboration.',
    workshops_section: 'Learning & Development',
    workshops_desc: 'Sharing practical knowledge, resources, and technical experience across the community.',
    growth_section: 'Community Growth',
    growth_desc: 'More than 1,700 members and growing every day.',
    timeline_title: 'Our Journey',
    video_title: 'Online CTF competition',
    video_desc: 'CTF competition highlights and community moments.',
  },
  collaborate: {
    title: 'Collaborate With Us',
    subtitle: 'Let\'s build the future of cyber together',
    description: 'CyberIsrael collaborates with universities, communities, organizations, companies, and individuals who share our vision of strengthening cyber education, accessibility, and innovation in Israel.',
    form_title: 'Get in Touch',
    form_subtitle: 'Fill out the form below and we\'ll be in touch.',
    name: 'Your Name',
    org: 'Organization',
    email: 'Email Address',
    message: 'Your Message',
    send: 'Send Message',
  },
  footer: {
    tagline: 'Building Israel\'s cyber future, one hacker at a time.',
    links: 'Quick Links',
    community: 'Community',
    legal: 'Legal',
    privacy: 'Privacy Policy',
    terms: 'Terms of Use',
    copyright: '© 2026 CyberIsrael. All rights reserved.',
  },
  theme: {
    dark: 'Dark Mode',
    light: 'Light Mode',
  },
  lang: {
    en: 'English',
    he: 'עברית',
  },
}

export default en
