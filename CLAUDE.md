...
- `src/services/articlesData.ts` is the article index/metadata store (a static in-memory array of `Article` objects — title, slug/`href`, category, tags, image, etc). `getArticleBySlug()` is the lookup used by pages. Two optional flags on an article drive placement: `featured` (wider card + FEATURED badge in the articles page grid) and `homePreview` (include it in the home page preview section — read through `getHomePreviewArticles()`, which keeps the declaration order).
